...
func (h *IgnoreHandler) createAttrTree(uniq *b.Tree[string, any], goas []*groupOrAttrs, depth int) {
	if len(goas) == 0 {
		return
	}
...
func (h *IgnoreHandler) resolveValues(uniq *b.Tree[string, any], attrs []slog.Attr, depth int) {
	var ok bool
	for _, a := range attrs {
